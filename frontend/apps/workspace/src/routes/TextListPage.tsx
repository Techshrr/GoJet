  ...
  const createMutation = useMutation({
    mutationFn: () => {
      ...
      return client.create(runtime.workspaceId, {
        ...
        password: password || undefined, expires_at: toRFC3339(expiresAt), one_time: oneTime,
    ...
    <WorkspaceShell state={readOnly && runtime ? 'read-only-role' : 'notification-attention'} sectionLabel="Text">
      <section className="text-page" data-page="text-list" data-state={pageState}>
        <header className="text-page-header">
          <div><p className="text-eyebrow">Text</p><h1>Text sharing</h1><p>Create private or public text resources with server-authoritative access, expiry and one-time lifecycle controls.</p></div>
        </header>
        ...
        {items.length > 0 ? (
          <div className="text-list" aria-label="Workspace Text shares">
            {items.map((item) => {
