...
func (a *WorkspaceAPIKeyHTTPAPI) list(w http.ResponseWriter, r *http.Request) {
	...
	if err != nil {
		apiKeyWriteAuthorityError(w, err)
		return
	}
...
func apiKeyDecodeJSON(w http.ResponseWriter, r *http.Request, dst any) bool {
	...
	if err := dec.Decode(dst); err != nil {
		apiKeyWriteError(w, http.StatusBadRequest, "invalid_json")
		return false
	}
	return true
}
func apiKeyWriteAuthorityError(w http.ResponseWriter, err error) {
	...
	case errors.Is(err, ErrInvalid):
		apiKeyWriteError(w, http.StatusBadRequest, "invalid_request")
	case errors.Is(err, ErrUnauthorized):
		apiKeyWriteError(w, http.StatusUnauthorized, "authentication_required")
	case errors.Is(err, ErrForbidden):
		apiKeyWriteError(w, http.StatusForbidden, "forbidden")
	case errors.Is(err, ErrNotFound):
		apiKeyWriteError(w, http.StatusNotFound, "not_found")
	case errors.Is(err, ErrConflict):
		apiKeyWriteError(w, http.StatusConflict, "conflict")
	case errors.Is(err, ErrRateLimited):
		apiKeyWriteError(w, http.StatusTooManyRequests, "rate_limited")
	default:
		apiKeyWriteError(w, http.StatusInternalServerError, "server_error")
	}
}
func apiKeyWriteError(w http.ResponseWriter, status int, code string) {
	apiKeyWriteJSON(w, status, map[string]any{"error": map[string]any{"code": code}})
}
func apiKeyWriteJSON(w http.ResponseWriter, status int, value any) {
