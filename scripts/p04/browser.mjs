...
async function inspectTarget(target, viewportName, viewport) {
  ...
  await page.screenshot({ path: `${capturesDir}/${file}`, fullPage: true });
...
for (const target of targets) {
  for (const [viewportName, viewport] of Object.entries(viewports)) await inspectTarget(target, viewportName, viewport);
}
...
async function verifySpa(url, selector, expectedPath) {
  ...
  const preserved = await page.evaluate((value) => window.__gojetP04Marker === value, marker);
  report.route_transitions.push({ url, selector, expectedPath, preserved, finalUrl: page.url() });
  await context.close();
}
