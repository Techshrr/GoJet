#!/usr/bin/env python3
"""GoJet V10 P07 exact-head closure validator for P07-T020."""
...
P07 = ROOT / "artifacts" / "v10" / "P07"
RESULTS = P07 / "results"
PLAN = P07 / "test-plan.json"
MANIFEST = P07 / "regression-manifest.json"
INDEX = P07 / "evidence-index.json"
T020 = RESULTS / "P07-T020.json"
...
def write_outputs(head: str, plan: dict[str, Any], manifest: dict[str, Any], entries: list[dict[str, Any]], errors: list[str]) -> None:
    ...
    t020 = {
        "case_id": "P07-T020",
        "name": "p00-p07-regression-and-evidence-index",
        "status": status,
...
def validate_written_index(head: str, expected_entries: list[dict[str, Any]], errors: list[str]) -> None:
    ...
    try:
        ...
        return
    require(index.get("node") == "P07", f"evidence index node={index.get('node')}", errors)
    ...
    require(index.get("input_evidence") == expected_entries, "evidence index input_evidence differs from validated evidence", errors)
    closure = index.get("closure_result")
    require(isinstance(closure, dict), "evidence index closure_result missing", errors)
    if isinstance(closure, dict):
        require(closure.get("case_id") == "P07-T020", "evidence index closure case mismatch", errors)
        require(closure.get("implementation_commit") == head, "evidence index closure commit mismatch", errors)
        require(closure.get("sha256") == sha256(T020), "evidence index closure digest mismatch", errors)


def main() -> int:
    parser = argparse.ArgumentParser()
    parser.add_argument("--case", required=True, choices=["P07-T020"])
    parser.parse_args()
    head = exact_head()
    errors: list[str] = []
    ...
    if errors:
        for error in errors:
            print(f"P07-T020: {error}")
        return 1
    print(f"P07-T020: PASS — 19/19 exact-head evidence inputs and {len(REQUIRED_WORKFLOWS)}/{len(REQUIRED_WORKFLOWS)} regression workflows green for {head}")
    return 0
