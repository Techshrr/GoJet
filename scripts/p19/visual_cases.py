...
def main():
    ...
    manifest=json.loads((SITE/'src/website/social-cards.json').read_text(encoding='utf-8'))
    errors=[]; cards={}
    for locale,record in manifest['cards'].items():
        ...
        if digest!=record['sha256']: errors.append(f'{locale}: social card digest mismatch {digest}')
    ...
    for page in pages:
        for locale,path,copy in [('en',page['path'],page['en']),('zh-CN',page['zhPath'],page['zh'])]:
            html=file_for(path).read_text(encoding='utf-8'); record=manifest['cards'][locale]; url=BASE+record['path']
            expected={
            ...
            metadata_checked+=1
    data={'node':'P19','case':'P19-T019','name':'Social cards and asset attribution','status':'PASS' if not errors else 'FAIL','implementation_commit':head(),'errors':errors,'details':{'schema':manifest['schema'],'cards':cards,'metadata_documents_checked':metadata_checked,'attribution':str(attribution.relative_to(ROOT)) if attribution.exists() else None}}
